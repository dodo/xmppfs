    ...
    switch(event) {
        ...
        case "mkdir":
            if (!(x.node instanceof fs.Directory))
                return defaultcallback.apply(this, __slice.call(arguments));
            var jid = new xmpp.JID(args[0]);
            var barejid = jid.bare().toString();
            var account = x.contact.account;
            x.node.mkdir(barejid, args[1], function (err) {
                if (err === fs.E.OK)
                    account.client.subscribe(
                        account.add(jid, x.node.children[barejid]).get('jid'));
                args[2](err);
            });
            break;
        default:
